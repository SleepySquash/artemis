...
Spec enumDefinitionToSpec(EnumDefinition definition) => CodeExpression(
  Code('''enum ${definition.name.namePrintable} {
...
String _enumValueToSpec(EnumValueDefinition value) {
  final annotations = value.annotations
      ...
      .followedBy(['@JsonValue(\'${value.name.name}\')'])
      .join(' ');
...
Method _propsMethod(Iterable<String> body) {
  return Method(
    (m) => m
      ...
      ..lambda = true
      ..body = Code(
        '[${body.mergeDuplicatesBy((i) => i, (a, b) => a).join(', ')}]',
      ),
  );
}
...
Spec generateLibrarySpec(LibraryDefinition definition) {
  ...
  importDirectives.addAll(
    definition.customImports.map(
      (customImport) => Directive.import(customImport),
    ),
  );
